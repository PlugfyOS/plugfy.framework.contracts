...
- **Accept:** L1 = only the nested `framework/` engine (+ the L1 `pipeline/runner` envelope it drives); loader/supervisor/resolver/discovery in Platform ✅; plugin/wasm/supervisor-proto in Foundation (L2 `plugfy.foundation.transport`) ✅; runner in L1 pipeline ✅; outer module retired ✅.
...
| **DC-1** ✅ | **CI decouple-check**: fail the build if any L1 engine package imports `github.com/PlugfyOS/plugfy.{foundation,platform}.*`. Wired in SW-6 — the `plugfy.framework.runtime` CI build job greps the engine module for those paths and fails on a hit (replacing the stale outer-module `decouple-check.sh`, removed when the outer module retired). Makes the SW-7 decision machine-checked rather than convention — DONE v1.12.25 | SW-7 | SW-7 |
