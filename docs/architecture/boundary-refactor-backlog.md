...
| micro-kernel loader | **L3** | ✅ DONE (SW-5, v1.12.22) — `plugfy.platform.kernel/kernel/loader` |
| supervisor | **L3** | ✅ DONE (SW-5, v1.12.22) — `plugfy.platform.kernel/kernel/supervisor` (incl. the `plugfy.supervisor.v1` genpb proto) |
| capability resolver + reconciler | **L3** | ✅ DONE (SW-5, v1.12.22) — `plugfy.platform.kernel/kernel/resolver` |
| supervisor-coupled service discovery (`ServiceIndex`/on-disk `Discovery`) | **L3** | ✅ DONE (SW-5, v1.12.22) — `plugfy.platform.kernel/kernel/discovery` (package renamed `registry`→`discovery`; imports the L2 `plugfy.foundation.registry` index/manifest one-way) |
...
| **SW-5** ✅ | Micro-kernel machinery → L3 (loader/supervisor/resolver + supervisor-coupled `ServiceIndex`/`Discovery` → `plugfy.platform.kernel/kernel/*`) — DONE v1.12.22 | NR-04 (host-composition half) ✅, bug #9 (half) | NR-01 physical relocation (SW-3) + SW-4 `installed`→L3; the runtime registry held ONLY the L3-bound supervisor-coupled index |
