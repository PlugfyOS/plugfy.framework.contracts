...
### NR-01 (P1) · Relocate Provider/Kind/registry out of L1 to Foundation — un-embed ✅ DONE (v1.12.16)
...
- **REMAINING (P2, future R5):** the physical relocation of `spi/provider.go` (`Provider`/`Kind`/`CapabilityRequirement`) and `runtime/registry` out of L1 into Foundation; `contracts` still exports them. The coupling that gated it is now broken; the move itself is still pending. Pruning the now-redundant `DefaultUnit` Provider helper methods is a separate **P3** hygiene cleanup (kept now so no caller breaks).
- **ABI:** breaking on `spi/core` (Unit shrank). The remaining `spi/provider` break lands with the relocation; golden ABI re-scoped fully by NR-08.
- **Accept (un-embed):** ✅ `core.Unit` no longer embeds `spi.Provider`. **Accept (relocation, pending):** `contracts` exports no Provider/Kind/registry; Foundation owns the provider contract + registry.
