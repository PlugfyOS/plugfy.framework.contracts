...
| per-Invoke Runner envelope | **L1** | ✅ DONE — rehomed to `pipeline/runner` (SW-6, v1.12.25) |
| standalone job runner + CLI + demo builtin | **L1** | `runtime/framework` (`cmd/plugfy`, `cli`, `job`, `builtin`) — the runtime repo's sole module after SW-6 |
...
| transport adapters (native/subprocess plugin tiers + wasm) + the supervisor wire proto | **L2** | ✅ DONE — relocated `runtime/plugin`, `runtime/wasm` + the `plugfy.supervisor.v1` proto → `plugfy.foundation.transport` (`plugin`, `wasm`, `supervisorwire`) (SW-6, v1.12.25) |
...
| micro-kernel loader | **L3** | ✅ DONE (SW-5, v1.12.22; namespace flattened v1.12.23) — `plugfy.platform.kernel/loader` |
| supervisor | **L3** | ✅ DONE (SW-5, v1.12.22; namespace flattened v1.12.23) — `plugfy.platform.kernel/supervisor` (incl. the `plugfy.supervisor.v1` genpb proto) |
| capability resolver + reconciler | **L3** | ✅ DONE (SW-5, v1.12.22; namespace flattened v1.12.23) — `plugfy.platform.kernel/resolver` |
| supervisor-coupled service discovery (`ServiceIndex`/on-disk `Discovery`) | **L3** | ✅ DONE (SW-5, v1.12.22; namespace flattened v1.12.23) — `plugfy.platform.kernel/discovery` (package renamed `registry`→`discovery`; imports the L2 `plugfy.foundation.registry` index/manifest one-way) |
...
### NR-04 (P1) · Relocate the runtime repo's OUTER micro-kernel module to L2/L3 ✅ DONE (host-composition half SW-5 v1.12.22; transport/runner half SW-6 v1.12.25 — outer module retired)
- **Where:** `plugfy.framework.runtime` — the OUTER go.mod (the repo root: `loader`, `supervisor`, `resolver`/reconciler, `plugin` tiers, `wasm`).
- **What:** the runtime repo has TWO go.mod modules (an undocumented L1/L3 seam — bug #9): the **nested `framework/` module** (the pipeline engine + job runner + demo builtin) is the **L1** core and STAYS; the **outer module** is host machinery — transport adapters (`plugin`, `wasm`) → **L2(adapters)**, loader + supervisor + resolver/reconciler + the supervisor-coupled discovery → **L3**. Split the repo along that seam.
- **DONE (host-composition half, SW-5, v1.12.22):** `loader`, `supervisor` (incl. the `plugfy.supervisor.v1` genpb proto, regenerated under the new go_package — never text-swept), `resolver`/reconciler, and the supervisor-coupled `registry` discovery (`services.go`/`discovery.go`, renamed package `registry`→`discovery`) all moved into **`plugfy.platform.kernel`** as `kernel/loader`, `kernel/supervisor`, `kernel/resolver`, `kernel/discovery`. The discovery package imports the L2 `plugfy.foundation.registry` index/manifest one-way (legal L3→L2) and the L3 supervisor proto (L3-internal). ~89 importers across ~20 repos flipped (server `internal/wiring` + the 16 system services + agent.engine that consumed the supervisor proto + the kernel itself + e2e tests); the system services + agent.engine, which imported ONLY the supervisor proto, now require `plugfy.platform.kernel` and dropped `plugfy.framework.runtime` entirely. **Inversion audit:** ZERO L1 packages (contracts, pipeline, the nested `framework/` engine, `runner`) import the moved machinery; the SW-4 transitional `loader → plugfy.platform.installed` edge is now L3→L3 ✓; `plugfy run` (the L1 engine) still runs standalone with no kernel dependency. **DONE (transport/runner half, SW-6, v1.12.25):** `plugin`/`wasm` + the `plugfy.supervisor.v1` proto relocated to L2 `plugfy.foundation.transport` (the proto regenerated under the new go_package — never text-swept); `runner` rehomed to L1 `plugfy.framework.pipeline/runner`. The L3 kernel supervisor imports the L2 proto and re-exports it at its established `supervisor/contracts/genpb/supervisorv1` path for its domain-service consumers (the 16 system services + agent.engine compile unchanged). The former outer-module→L3 supervisor-proto edge is GONE: plugin(L2)→proto(L2) and kernel-supervisor(L3)→proto(L2), ZERO L2→L3, ZERO cycle. The OUTER module is retired — the runtime repo now contains only the nested L1 `framework/` engine.
- **Accept:** L1 = only the `plugfy.framework.runtime` engine (+ the L1 `pipeline/runner` envelope it drives); loader/supervisor/resolver/discovery in Platform ✅; plugin/wasm/supervisor-proto in Foundation (L2 `plugfy.foundation.transport`) ✅; runner in L1 pipeline ✅; outer module retired ✅.
- **Follow-on #117 (PHASE 2, v1.12.27) — nested-module flatten ✅ DONE:** with the outer module retired, the repo's sole module was the NESTED `framework/` one, whose path was the confusing double-"framework" `github.com/PlugfyOS/plugfy.framework.runtime/framework` (the same smell as the fixed `plugfy.platform.kernel/kernel/*`). Promoted that module to the repo ROOT so the path is the clean, logical `github.com/PlugfyOS/plugfy.framework.runtime`, packages `.../runtime/{job,cli,builtin,cmd/plugfy}`. Internal imports flattened; the two importers (`plugfy.foundation`, `plugfy.platform.server`) flipped to the new path; `plugfy.foundation.examples` dropped the stale dissolved-runtime dep by moving to the current SDK. A dedicated DC-1 guard (`scripts/decouple-check.sh` + `decouple-check.yml`) was added to the runtime, mirroring contracts+pipeline — PASS locally (the engine imports only L1 contracts+pipeline). Lockstep v1.12.27 across runtime→foundation→examples→server→qa; clean-cache GOWORK=off qa/smoke green; `plugfy run` standalone green. A documented, CI-verified decision — not an inviolable rule.
...
| 9 | **Undocumented two-go.mod L1/L3 seam** in the runtime repo — the nested `framework/` (L1) and the outer module (host machinery) shared a repo with no stated boundary. | was `runtime/go.mod` + `runtime/framework/go.mod` | **✅ RESOLVED (SW-6, v1.12.25)** via **NR-04**: SW-5 moved the L3 host-composition machinery to `plugfy.platform.kernel`; SW-6 then moved the transport adapters + supervisor proto to L2 `plugfy.foundation.transport` and `runner` to L1 `plugfy.framework.pipeline`, and **retired the OUTER go.mod**. The repo now has a SINGLE module — the nested L1 `framework/` engine — so the ambiguous two-module seam is gone. |
...
| **SW-5** ✅ | Micro-kernel machinery → L3 (loader/supervisor/resolver + supervisor-coupled `ServiceIndex`/`Discovery` → `plugfy.platform.kernel/*`) — DONE v1.12.22; **namespace flattened v1.12.23** (the redundant `kernel/kernel/*` double-segment removed — packages now sit at the kernel module root; #102) | NR-04 (host-composition half) ✅, bug #9 (half) | NR-01 physical relocation (SW-3) + SW-4 `installed`→L3; the runtime registry held ONLY the L3-bound supervisor-coupled index |
| **SW-6** ✅ | Transport adapters relocated (plugin/wasm → L2 `plugfy.foundation.transport`; the `plugfy.supervisor.v1` proto → L2 `plugfy.foundation.transport/supervisorwire`, regenerated under the new go_package — never text-swept; runner → L1 `plugfy.framework.pipeline/runner`); the L3 kernel supervisor imports the L2 proto and re-exports it at its established path for domain-service consumers, dissolving the outer-module→kernel supervisor-proto edge (now plugin(L2)→proto(L2) and kernel(L3)→proto(L2), ZERO L2→L3, ZERO cycle); the runtime repo's OUTER module retired, leaving only the nested L1 `framework/` engine — DONE v1.12.25 | NR-04 (adapter half), bug #2 | SW-5 |
| **SW-7** ✅ (partial) | **L1 self-containment** — the L1 framework depends on NOTHING from foundation/platform. Closed the two SW-3 residual pipeline→L2 edges: un-embed the L2 `Provider` from `spi.PipelineEngine` (engine contract is just `Run`; `Name`/`Kind`/`Capabilities`/`HealthCheck` stay as descriptor-derived helpers; `KindPipeline`→L1 `core.Kind`), and move the engine-as-registry-Provider self-registration OUT of L1 to the composition root (platform server wiring). `go mod tidy` drops `plugfy.foundation.{registry,sdk}` from pipeline + nested framework. `go list -deps` proves ZERO foundation/platform imports across contracts/pipeline/nested-framework/runner — DONE v1.12.23 (#102) | NR-01 (un-embed precedent), R6 (this is the Provider-edge half of R6) | SW-3, SW-5 |
| **SW-7c** ⏳ | The pipeline still pulls the `cel-go` (`github.com/google/cel-go` + `cel.dev/expr`) third-party CEL evaluator (an L1-stdlib-only concern, NOT a foundation/platform module dep). Port/replace to keep L1 stdlib-only. Separate from SW-7's foundation/platform decoupling | R6 (engine de-domain) | SW-7 |
| **DC-1** ✅ | **CI decouple-check**: fail the build if any L1 engine package imports `github.com/PlugfyOS/plugfy.{foundation,platform}.*`. Wired in SW-6 — the `plugfy.framework.runtime` CI build job greps the engine module for those paths and fails on a hit (replacing the stale outer-module `decouple-check.sh`, removed when the outer module retired). #117 (v1.12.27) added the dedicated `scripts/decouple-check.sh` + `decouple-check.yml` to the (now flat-root) runtime, mirroring the ones on contracts+pipeline (`go list -deps` transitive check), PASS locally. Makes the SW-7 decision machine-checked rather than convention — DONE v1.12.25, guard added v1.12.27 | SW-7 | SW-7 |
| **#117** ✅ | **Nested-module flatten (PHASE 2)** — promote the runtime's nested `framework/` module to the repo ROOT so the path is the clean `github.com/PlugfyOS/plugfy.framework.runtime` (kills the confusing double "framework", same smell as the fixed kernel/kernel/*); flip the two importers; drop the stale dissolved-runtime dep in examples (move to current SDK); add the DC-1 guard above. Lockstep v1.12.27 runtime→foundation→examples→server→qa; clean-cache qa/smoke green — DONE v1.12.27 | NR-04 (outer-module retire), DC-1 | SW-6 |
