...
### NR-04 (P1) · Relocate the runtime repo's OUTER micro-kernel module to L2/L3 — host-composition half ✅ DONE (SW-5, v1.12.22)
...
| 9 | **Undocumented two-go.mod L1/L3 seam** in the runtime repo — the nested `framework/` (L1) and the outer module (host machinery) share a repo with no stated boundary. | `runtime/go.mod` + `runtime/framework/go.mod` | **PARTIALLY RESOLVED (SW-5, v1.12.22)** via **NR-04**: the L3 host-composition machinery (loader/supervisor/resolver/discovery) left the outer module for `plugfy.platform.kernel`; the outer module + its README/decouple-check now document the seam explicitly (outer = transport adapters awaiting SW-6; nested `framework/` = the L1 engine). Fully closed when SW-6 retires the outer module. |
...
| **SW-6** | Transport adapters relocate (plugin/wasm → L2; runner → L1 `plugfy.framework.pipeline`); dissolves the outer-module→kernel supervisor-proto edge; the runtime repo's outer module retires | NR-04 (adapter half), bug #2 | SW-5 |
...
| **DC-1** (follow-up) | **CI decouple-check**: fail the build if any L1 module (`framework.contracts`, `framework.pipeline`, `framework.runtime/framework`, `runner`) imports `github.com/PlugfyOS/plugfy.{foundation,platform}.*`. Makes the SW-7 invariant machine-enforced rather than convention | SW-7 | SW-7 |
