...
| `persistence` | the dialect-aware data plane `SQLDB`/`Tx`/`Rows`/`Row`/`Result` over the stdlib `database/sql`, with `Dialect` (postgres/sqlite), `Rebind` and the `Now`/`JSONExtract`/`Upsert` fragment helpers, the per-dialect migration runner (`MigrationSet`{Postgres,SQLite} + `ApplyMigrations`), plus the namespaced control-plane `RegistryStore`. A store authors DML once (`$N`) and DDL twice, and applies it through this seam without importing a concrete driver. Contracts only — engines stay in provider repos |
